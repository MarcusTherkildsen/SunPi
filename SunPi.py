...
def connect2plotly():
    ...
    # Comment this line (put # in front) to keep old data in plot when restarting
...
full_day_data = np.empty([2*num_of_points_12_hours,2*num_of_points_12_hours]) # 400 points when having 216 sec in between
...
while True:
    ...
    # Battery sensor
        try:
            bat_vol[k] = (ina.getBusVoltage_V() + (ina.getShuntVoltage_mV()/1000)) # Voltage
            bat_wat[k] = -bat_vol[k]*ina.getCurrent_mA()/1000 #bat_vol[k]*amp_temp,  Wattage
        except Exception:
    ...
    if day_data_collected_from < (datetime.now() + timedelta(hours=add_hour)).date():
        full_day_data = [np.copy(time_for_saving),np.copy(bat_wat_plotly),np.copy(bat_vol_plotly)]
        ...
        try:
            ...
            pass
           # unable to get sun curve for today. oh, well. we will try again tomorrow !
